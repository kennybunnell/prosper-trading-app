...
export function TrialStatusBanner() {
  const { user } = useAuth();
  const { data: userInfo } = trpc.user.getSubscriptionStatus.useQuery();
  ...
  const trialEndsAt = userInfo?.trialEndsAt ? new Date(userInfo.trialEndsAt) : null;
  const now = new Date();
  ...
  // Get scan usage
  const scansUsed = userInfo?.scansUsed || 0;
  const scansLimit = userInfo?.scansLimit || 10;
